...
> The file contains multiallelic variants, which cannot be stored in a .bim.We need to add --max-alleles 2 in order to avoid an error.
```
...
```
> --recode was used to create a .map file, but is now deprecated. We choose to build a .vcf file instead.

The datasets now contain the exact same variants, but they need to have the same build.

## Change the build 1000 Genomes data build
```
...
```
> output: 1kG_PCA8.pvar, 1kG_PCA8.psam, 1kG_PCA8.pgen
```
...
```
> output: PCA_merge2.eigenval, PCA_merge2.eigenvec
...
Perform an PCA ONLY on HapMap data without ethnic outliers. The values of the 10 PCA dimensions are subsequently used as covariates in the association analysis in the third tutorial.
```
plink2 --bfile HapMap_3_r3_12 --extract indepSNP.prune.in --pca 10 --out HapMap_3_r3_13_pca
```

> Save a plink covariate file.
```
