...
> By default, females must be under 0 and males above 1 which just does not work. Therefore we need to specify our thresholds.
```
...
The following two scripts can be used to deal with individuals with a sex discrepancy.Note, please use **one of the two** options below to generate the bfile HapMap_3_r3_6, this file we will use in the next step of this tutorial.
...
> By default the --hwe option in plink used to only filter for controls. However for PLINK 2.0, this is no longer the case. There is currently no special handling of case/control phenotypes. If needed, we can use --keep-if "PHENO1==control" or "PHENO1==case".
...
## Step 6 : Check for cryptic relatedness
...
> The original tutorial was checking the pihat values for each pair. However I was unsuccessful in converting the commands.
